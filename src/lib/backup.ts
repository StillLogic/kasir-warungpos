...
export async function exportBackup(): Promise<Blob> {
  ...
  const debtPayments = JSON.parse(localStorage.getItem('db_debt_payments') || '[]');
  const markupRules = JSON.parse(localStorage.getItem('warungpos_markup_rules') || '[]');
  const storeSettings = JSON.parse(localStorage.getItem('store-settings') || '{}');
...
export async function importBackup(file: File): Promise<{ success: boolean; message: string; itemCounts?: Record<string, number> }> {
  try {
    ...
    if (data.data.debtPayments) {
      localStorage.setItem('db_debt_payments', JSON.stringify(data.data.debtPayments));
      itemCounts.debtPayments = (data.data.debtPayments as unknown[]).length;
    }
    
    if (data.data.markupRules) {
      localStorage.setItem('warungpos_markup_rules', JSON.stringify(data.data.markupRules));
      itemCounts.markupRules = (data.data.markupRules as unknown[]).length;
    }
    
    if (data.data.storeSettings && Object.keys(data.data.storeSettings as object).length > 0) {
      localStorage.setItem('store-settings', JSON.stringify(data.data.storeSettings));
    }
...
export async function getStorageStats(): Promise<{
  ...
  const categories = (JSON.parse(localStorage.getItem('db_categories') || '[]') as unknown[]).length;
  const customers = (JSON.parse(localStorage.getItem('db_customers') || '[]') as unknown[]).length;
  const debts = (JSON.parse(localStorage.getItem('db_debts') || '[]') as unknown[]).length;
